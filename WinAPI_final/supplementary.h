...
LPCWSTR StringToLPCWSTR(const std::string& str) {
    int size_needed = MultiByteToWideChar(
        CP_UTF8,            // Êîäèðîâêà èñõîäíîé ñòðîêè (UTF-8)
        0,                  // Ôëàãè (îáû÷íî 0)
        str.c_str(),        // Èñõîäíàÿ ñòðîêà
        (int)str.size(),   // Äëèíà ñòðîêè
        nullptr,           // Íå íóæåí áóôåð (òîëüêî ðàñ÷¸ò ðàçìåðà)
        0                  // Ðàçìåð áóôåðà (ïîêà 0)
    );

    if (size_needed == 0) {
        return L""; // Îøèáêà êîíâåðòàöèè
    }
    ...
    wideStr[size_needed] = L'\0'; // Äîáàâëÿåì íóëåâîé ñèìâîë

    return wideStr; // Âîçâðàùàåì óêàçàòåëü (íå çàáûòü îñâîáîäèòü ïàìÿòü!)
}
